...
/// Flujo "Pago final": el cliente elige cómo liquidará su unidad - recursos
/// propios (STP) o crédito hipotecario. La elección se persiste vía
/// cliente-pago-final; con banco preferente sigue la precalificación.
...
class _PagoFinalScreenState extends ConsumerState<PagoFinalScreen> {
  ...
  /// estimación), para mostrarlo en el estatus.
  ...
  void initState() {
    ...
    if (widget.tipoFinanciamiento == 'CREDITO_HIPOTECARIO') {
      ...
      if (widget.solicitud != null) {
        ...
        // Ya eligió crédito pero aún no envía solicitud: va al selector de
        // banco, nunca a datos de pago.
        _paso = _Paso.banco;
        _cargarBancos();
      }
    }
  ...
  /// Confirma el banco elegido (solo bancos con convenio):
    ...
    try {
      await ref
          ...
          .setFinalPaymentMethod(
            accountId: widget.cuentaId,
            method: _metodo!,
            bankId: _metodo == 'CREDITO_HIPOTECARIO' ? _idBanco : null,
          );
      ref.invalidate(propertyDetailProvider(widget.cuentaId));
      if (!mounted) return;
  ...
  List<Widget> _cuerpo(SozuColorRoles tone) {
    // Unidad 100% pagada: no hay nada que elegir.
  ...
  /// Resumen de la precalificación enviada en esta sesión.
