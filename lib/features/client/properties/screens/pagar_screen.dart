...
/// Pagar un acuerdo, sin crédito hipotecario: paso 1 = saldo a liquidar +
/// método "Recursos propios (STP)"; paso 2 = datos para pago (monto,
/// vencimiento, CLABE, beneficiario, concepto).
...
class _PagarScreenState extends ConsumerState<PagarScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget cuerpo(SozuColorRoles tone) => FutureBuilder<DatosPago>(
      future: _datos,
      builder: (context, snap) {
        ...
        // a las instrucciones de transferencia STP.
        if (portal) return _portalInstrucciones(tone, d);
        return _instrucciones ? _paso2(tone, d) : _paso1(tone, d);
      },
    );

    // Modo portal: fuerza el tema claro (evita cards oscuras dentro del shell
    // claro) y centra el flujo a máx. 640px.
    if (portal) {
      return Theme(
        data: sozuLightTheme(),
        child: Builder(
  ...
  // Instrucciones de pago STP (modo portal): banner de confirmación automática,
  // monto/vencimiento, CLABE, banco receptor, beneficiario, concepto y CTA
  // "Copiar CLABE".
      ...
      children: [
        ...
        SCard(
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Icon(Icons.shield_outlined, size: 20, color: tone.fgMuted),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
  ...
  // Fila de dato: label uppercase + valor y copiar opcional, en card blanca.
  ...
  Widget _paso1(SozuColorRoles tone, DatosPago d) {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        Text(
  ...
  Widget _paso2(SozuColorRoles tone, DatosPago d) {
    final sinClabe = (d.clabe ?? '').isEmpty;
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
