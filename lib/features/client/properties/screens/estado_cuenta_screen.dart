...
import 'package:sozu_cliente_app/features/client/properties/components/recibo_pago_sheet.dart';
...
class _EstadoCuentaScreenState extends ConsumerState<EstadoCuentaScreen> {
  ...
  Widget _resumen(SozuColorRoles tone, PropiedadCard c, EstadoCuenta d) {
    ...
    // Chip: SIEMPRE el estatus real de la propiedad (etapa),
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // MODO PORTAL (web ≥1024). Solo capa visual: reutiliza providers, filtros
  // (_estatus/_anio) y acciones existentes.
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  /// Fondo del encabezado de la tabla y de la fila expandida (muted aplanado).
  static const Color _theadBg = Color(0xFFFDFEFE);

  /// Fecha corta es-MX: "15 jul 2026".
  ...
  /// Colores del cuadro de unidad del selector, por estatus.
  ...
  /// Movimientos: los acuerdos del plan de pagos; sin plan, los pagos
  /// realizados.
  ...
  /// Abre el recibo in-app de un movimiento, degradando concepto/propiedad a
  /// lo disponible. El PDF sale por [_generarReciboPdf] o por el propio sheet.
  ...
  /// Genera el recibo PDF vía backend.
  ...
  /// Sub-fila de pago aplicado: regla verde a la
  ...
  Widget _portalResumen(PropiedadCard c, EstadoCuenta d) {
    ...
    final pendientes = d.acuerdos.where((a) => !a.pagadoCompleto).toList()
      ..sort((a, b) => (a.fecha ?? '').compareTo(b.fecha ?? ''));
    final proxima = pendientes.isEmpty ? null : pendientes.first;
    // Chip = estatus REAL de la propiedad ("En Preventa"/"Pago Pendiente"/
    // "Entregada"…), no un derivado del saldo. Mismo mapeo de color que el
    // selector: warning si pendiente, si no primary.
...
/// Estatus de un movimiento de la tabla.
enum _MovStatus { pagado, parcial, pendiente }

/// Movimiento de la tabla: acuerdo del plan de pagos o, sin plan, un pago
/// realizado.
class _Mov {
  final String? fecha;
